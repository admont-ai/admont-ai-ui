...
function ZoomControls() {
  ...
  return (
    <div className="absolute right-3 top-3 z-10 flex flex-col gap-1">
      <button className={btn} title="Zoom in" onClick={() => zoomIn(0.1)}>
        <ZoomIn className="size-3.5" />
      </button>
      <button className={btn} title="Zoom out" onClick={() => zoomOut(0.1)}>
  ...
  const mainContent = editing ? (
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden">
        {blobUrl && (
          <TransformWrapper minScale={0.2} maxScale={8} wheel={{ step: 0.015 }} centerOnInit>
            <ZoomControls />
            <TransformComponent
